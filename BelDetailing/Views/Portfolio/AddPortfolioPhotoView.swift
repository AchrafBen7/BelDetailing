...
struct AddPortfolioPhotoView: View {
    let providerId: String
    let engine: Engine
    let onPhotoAdded: () -> Void
    
    @Environment(\.dismiss) private var dismiss
    @StateObject private var viewModel: AddPortfolioPhotoViewModel
    ...
    init(providerId: String, engine: Engine, onPhotoAdded: @escaping () -> Void) {
        self.providerId = providerId
        self.engine = engine
        self.onPhotoAdded = onPhotoAdded
        _viewModel = StateObject(wrappedValue: AddPortfolioPhotoViewModel(providerId: providerId, engine: engine))
    }
            ...
            ScrollView(showsIndicators: false) {
                VStack(spacing: 24) {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        TextField("Ex: Nettoyage intérieur complet", text: $caption)
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Picker("Catégorie", selection: $serviceCategory) {
                            Text("Aucune").tag(ServiceCategory?.none)
                            ForEach(ServiceCategory.allCases, id: \.self) { category in
                                Text(category.localizedTitle).tag(ServiceCategory?.some(category))
                            }
                        }
                        .pickerStyle(.menu)
                    }
...
@MainActor
final class AddPortfolioPhotoViewModel: ObservableObject {
    ...
    func addPhoto(image: UIImage?, caption: String?, serviceCategory: ServiceCategory?) async -> Bool {
        ...
        let addResult = await engine.providerPortfolioService.addPhoto(
