...
struct CustomTabBar: View {
  ...
  private let items: [TabItem] = [
    ...
    .init(tab: .offers,   systemName: "briefcase.fill",             a11yLabel: R.string.localizable.tabOffers()),
    .init(tab: .profile,  systemName: "person.crop.circle.fill",    a11yLabel: R.string.localizable.tabProfile())
  ]

  var body: some View {
    ZStack {
      // Pilule noire derrière
      RoundedRectangle(cornerRadius: 28, style: .continuous)
        .fill(Color.black)
        .overlay(RoundedRectangle(cornerRadius: 28).stroke(.white.opacity(0.12), lineWidth: 0.7))
        ...
        ForEach(items) { item in
          Button {
            ...
            Image(systemName: item.systemName)
              ...
              .frame(width: 44, height: 44) // hit-target carré
              .background(
                selection == item.tab
          ...
          .buttonStyle(.plain)
          .frame(maxWidth: .infinity) // espace égal pour chaque icône
